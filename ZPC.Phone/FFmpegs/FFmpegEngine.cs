...
namespace ZPC.Phone
{
    //https://github.com/0voice/audio_video_streaming 资料地址
    public class FFmpegEngine
    {
        ...
        public void Convert(MediaInfo sourceInfo, string outputPath, ConversionOptions conversionOptions)
        {
            ...
            // 获取输出的总帧数
            ZPC.Phone.FpsFilter fpsFilter = conversionOptions.Filters.FirstOrDefault(f => f is ZPC.Phone.FpsFilter) as ZPC.Phone.FpsFilter;
        ...
        private string BuildArgumentsString(MediaInfo sourceInfo, string destination, ConversionOptions conversionOptions, TimeSpan start, TimeSpan end, string twopasslogfile = "", bool fadeStart = false, bool fadeEnd = false)
        {
            ...
            if (conversionOptions.EncodingMode != EncodingMode.AverageBitrate_FirstPass)
            {
                ...
                if (fadeStart)
                {
                    filtergraph.AddFilter(new ZPC.Phone.FadeFilter(ZPC.Phone.FadeMode.In, 0.2), 0, 0);
                }
                if (fadeEnd)
                {
                    TimeSpan actualEnd = end != TimeSpan.Zero ? end : sourceInfo.Duration;
                    filtergraph.AddFilter(new ZPC.Phone.FadeFilter(ZPC.Phone.FadeMode.Out, 0.2, actualEnd.TotalSeconds - start.TotalSeconds - 0.2), 0, 0); 
                }
                        ...
                        */
                        switch (key)
                        {
                            ...
                            case "out_time":
                                ...
                                break;
                            case "speed":
                                progressData.EncodingSpeed = Single.Parse(value.Substring(0,value.Length-1));
                                break;
                            case "progress":
                                synchronizationContext.Post(new SendOrPostCallback((o) =>
                                {
